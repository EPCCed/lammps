...
PairSHRESquared::PairSHRESquared(LAMMPS *lmp) : Pair(lmp),
...
PairSHRESquared::~PairSHRESquared()
{
  if (allocated) {
...
void PairSHRESquared::compute(int eflag, int vflag)
{
...
void PairSHRESquared::settings(int narg, char **arg)
{
...
void PairSHRESquared::coeff(int narg, char **arg)
{
  if (narg < 10 || narg > 11)
...
void PairSHRESquared::init_style()
{

//  avec = (AtomVecEllipsoid *) atom->style_match("ellipsoid");
//  if (!avec) error->all(FLERR,"Pair resquared requires atom style ellipsoid");

  avec = (AtomVecSpherharmUnitTests *) atom->style_match("spherharmunittests");
  if (!avec) error->all(FLERR,"Pair resquared requires atom style spherharmunittests");
...
//      error->all(FLERR,"Pair resquared requires atoms with same type have same shape");
    if (!shape_consistency(i,shape1[i][0],shape1[i][1],shape1[i][2]))
      error->all(FLERR,"Pair resquared requires atoms with same type have same shape");
    if (setwell[i]) {
...
int PairSHRESquared::shape_consistency(int itype,
...
//  double *shape;
  ...
  auto avec_sphperatom= (AtomVecSpherharmUnitTests *) atom->style_match("spherharmunittests");

  int flag = 0;
  for (int i = 0; i < atom->nlocal; i++) {
    if (atom->type[i] != itype) continue;
//    if (ellipsoid[i] < 0) shape = zero;
//    else shape = bonus[ellipsoid[i]].shape;
    else {
      avec_sphperatom->get_shape(i, shape[0], shape[1], shape[2]);
    }
...
double PairSHRESquared::init_one(int i, int j)
{
...
void PairSHRESquared::write_restart(FILE *fp)
{
...
void PairSHRESquared::read_restart(FILE *fp)
{
...
void PairSHRESquared::write_restart_settings(FILE *fp)
{
...
void PairSHRESquared::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairSHRESquared::precompute_i(const int i,RE2Vars &ws)
{
...
//  MathExtra::quat_to_mat_trans(bonus[ellipsoid[i]].quat,ws.A);
  ...
  MathExtra::quat_to_mat_trans(quat[i],ws.A);
...
//  std::cout << "ws.lAsa[i] " << i << " " <<quat[i][0]<<" "<<quat[i][1]<<" "<<quat[i][2]<<std::endl;
//  std::cout << "ws.lAsa[i] " << i << " " <<ws.A[0][0]<<" "<<ws.A[0][1]<<" "<<ws.A[0][2]<<std::endl;
//  std::cout << "ws.lAsa[i] " << i << " " <<ws.lAsa[0][0][0]<<" "<<ws.lAsa[0][0][1]<<" "<<ws.lAsa[0][0][2]<<std::endl;
...
double PairSHRESquared::det_prime(const double m[3][3], const double m2[3][3])
{
  double ans;
...
double PairSHRESquared::resquared_analytic(const int i, const int j,
...
double PairSHRESquared::resquared_lj(const int i, const int j,
