...
#include "compute_shcoeffperatom.h"
#include <mpi.h>
#include "atom.h"
#include "update.h"
#include "force.h"
...
ComputeShcoeffperatom::ComputeShcoeffperatom(LAMMPS *lmp, int narg, char **arg) :
        Compute(lmp, narg, arg)
{
  ...
  numshtypes = size_array_rows;
//  size_array_cols = 2 * (maxshexpan + 1) * (maxshexpan + 1);
  size_array_cols = (maxshexpan+1)*(maxshexpan+2);
  size_vector = maxshexpan+1;
  memory->create(array, size_array_rows, size_array_cols, "ComputeShcoeffperatom:shcoeff");
  memory->create(vector, size_vector, "ComputeShcoeffperatom:inertia_scalefactor");
...
ComputeShcoeffperatom::~ComputeShcoeffperatom()
{
...
void ComputeShcoeffperatom::calcexpansionfactors(int numpoints)
{
...
//  memory->create(vector, nmax + 1, "ComputeShcoeffperatom:scalefactor");
...
double ComputeShcoeffperatom::plegendre(const int l, const int m, const double x) {
...
double ComputeShcoeffperatom::plegendre_nn(const int l, const double x, const double Pnm_nn) {
...
double ComputeShcoeffperatom::plegendre_recycle(const int l, const int m, const double x, const double pnm_m1, const double pnm_m2) {
...
void ComputeShcoeffperatom::read_coeffs(char *filename)
{
...
void ComputeShcoeffperatom::read_coeffs()
{
  int m,n;
...
void ComputeShcoeffperatom::reallocate(char *&str, int &max, int n)
{
  if (n) {
...
int ComputeShcoeffperatom::numtriple(char *line)
{
  int count = 0;
  char *ptr = line;
...
------------------------------------------------------------------------- */
void ComputeShcoeffperatom::parse()
{
...
char *ComputeShcoeffperatom::nextword(char *str, char **next)
{
  char *start,*stop;
...
double ComputeShcoeffperatom::besseljzero(int k)
{
  if(k > 20)
  {
...
double ComputeShcoeffperatom::besselj1squared(int k)
{
  if(k > 21)
  {
    double x = 1.0/(k-0.25);
...
ComputeShcoeffperatom::QuadPair ComputeShcoeffperatom::GLPairS(size_t n, size_t k)
{
...
ComputeShcoeffperatom::QuadPair ComputeShcoeffperatom::GLPairTabulated(size_t l, size_t k)
{
  // Odd Legendre degree
  if(l & 1)
  {
...
ComputeShcoeffperatom::QuadPair ComputeShcoeffperatom::GLPair(size_t n, size_t k)
{
...
void ComputeShcoeffperatom::get_quadrature_values(int num_quadrature, int maxshexpan) {

  int num_quad2 = num_quadrature*num_quadrature;

  memory->create(angles, 2, num_quad2, "ComputeShcoeffperatom:angles");
  memory->create(weights, num_quadrature, "ComputeShcoeffperatom:weights");
  memory->create(quad_rads, numshtypes, num_quad2, "ComputeShcoeffperatom:quad_rads");
  ...
  for (int sht = 0; sht < numshtypes; sht++) {
    for (int k = 0; k < num_quad2; k++) {
      theta = angles[0][k];
      phi = angles[1][k];
      x_val = std::cos(theta);
      rad_val = array[sht][0] * std::sqrt(1.0 / (4.0 * MY_PI));
      Pnm_m2.clear();
      Pnm_m1.clear();
      for (int n = 1; n <= maxshexpan; n++) {
        ...
        if (n == 1) {
          ...
          Pnm_m2[0] = P_n_m;
          rad_val += array[sht][4] * P_n_m;
          P_n_m = plegendre(1, 1, x_val);
          Pnm_m2[1] = P_n_m;
          mphi = 1.0 * phi;
          rad_val += (array[sht][2] * cos(mphi) - array[sht][3] * sin(mphi)) * 2.0 * P_n_m;
        } else if (n == 2) {
          P_n_m = plegendre(2, 0, x_val);
          Pnm_m1[0] = P_n_m;
          rad_val += array[sht][10] * P_n_m;
          for (int m = 2; m >= 1; m--) {
            P_n_m = plegendre(2, m, x_val);
            Pnm_m1[m] = P_n_m;
            mphi = (double) m * phi;
            rad_val += (array[sht][nloc] * cos(mphi) - array[sht][nloc + 1] * sin(mphi)) * 2.0 * P_n_m;
            nloc += 2;
          }
          Pnm_nn = Pnm_m1[2];
        } else {
          ...
          for (int m = 1; m < n - 1; m++) {
            ...
            Pnm_m1[m] = P_n_m;
            mphi = (double) m * phi;
            rad_val += (array[sht][loc] * cos(mphi) - array[sht][loc + 1] * sin(mphi)) * 2.0 * P_n_m;
            loc -= 2;
          }
...
void ComputeShcoeffperatom::compute_volume(int num_quadrature) {
  ...
  for (int sht = 0; sht < numshtypes; sht++) {
    ...
    for (int i = 0; i < num_quadrature; i++) {
      for (int j = 0; j < num_quadrature; j++) {
        ...
        v1 *= std::pow(quad_rads[sht][count], 3.0);
...
void ComputeShcoeffperatom::getI(int num_quadrature) {

  memory->create(pinertia, numshtypes, 3, "ComputeShcoeffperatom:pinertia");
  memory->create(orient, numshtypes, 9, "ComputeShcoeffperatom:orient");
  ...
  std::cout << "NO SH TYPES = " << numshtypes << std::endl;
  ...
  for (int sht = 0; sht < numshtypes; sht++) {
    ...
    MathExtra::exyz_to_q(ex,ey,ez,orient[sht]);

    pinertia[sht][0] = inertia[0];
    pinertia[sht][1] = inertia[1];
    pinertia[sht][2] = inertia[2];
...
void ComputeShcoeffperatom::setatomprops() {
  int nlocal = atom->nlocal;
  int *mask = atom->mask;
  double **shcoeffs = atom->shcoeffs;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      type = shtype[i];
      for (int m = 0; m < 882; m++) {
        shcoeffs[i][m] = array[type][m];
      }
      inertia[i][0] = pinertia[type][0];
      inertia[i][1] = pinertia[type][1];
      inertia[i][2] = pinertia[type][2];
      quat[i][0] = orient[type][0];
      quat[i][1] = orient[type][1];
      quat[i][2] = orient[type][2];
      quat[i][3] = orient[type][3];
    }
  }
}
